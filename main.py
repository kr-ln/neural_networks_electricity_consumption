...
plt.plot(dates_test[:200], pred_test_actual[:200], label="Prediction")
plt.title("Prediction vs Real (Test) – Real Dates")
...
plt.figure(figsize=(12,4))
plt.plot(future_dates, future_actual, label="Future Demand", marker='o')

# Añadir etiquetas sobre cada punto
for dt, val in zip(future_dates, future_actual.flatten()):
    plt.text(dt, val + 100, f"{val:.0f}", ha='center', va='bottom', fontsize=8, rotation=90)
plt.title("Electricity demand forecast for the next 48 hours (actual dates with labels)")
plt.xlabel("Date and Time")
plt.ylabel("Electric Demand")
plt.xticks(rotation=45)
plt.legend()
